...
//init esaylogging 
INITIALIZE_EASYLOGGINGPP int64_t time_get()
{
...
int main(int argc, char *argv[]) {
    ...
    for (auto i = 0; i < num_of_client; ++i) {
        ...
        if (!result) {
            LOG(ERROR) << "connect control server error:[" << remote_addr.
              sin_addr.s_addr << ":" << remote_addr.sin_port << "]";
            exit(-1);
        }
    }
/*
    ...
    while (1) {
        ...
        for (auto i = 0; i < nfds; i++) {
            ...
            if (events[i].events & EPOLLIN)     //读
            {
                ...
                LOG(INFO) << "event = " << events[i].
                  events << " on fd =" << events[i].data.
                  fd << " , recv data_header [" << header_recv->
                  type << "," << header_recv->subtype << "," << header_recv->
                  len << "], n:" << n;
                ...
                LOG(INFO) << "event = " << events[i].
                  events << " on fd =" << events[i].data.
                  fd << " recv header_len + data_len [" << n + m << "]";
